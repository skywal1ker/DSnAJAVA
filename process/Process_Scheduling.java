...
public class Process_Scheduling {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        for (int i = 0; i < D.size(); i++) {System.out.println(D.get(i));} // Here it is going to show main information about process
        System.out.println();
        ...
        while (!Q.isEmpty()) {
            ...
            if (!running) {runningP = Q.poll();//deleting element according priority
                currentTime -= 1;                                   // here we need deduct by -1 to get desired output
                waitTime = currentTime - runningP.getArrivalTime(); // calculating wait time
                averageWaitTime += waitTime;                        // collecting whole time
                endTime = currentTime + runningP.getDuration();     // calculating endTime
            ...
            if (running && endTime == currentTime) {System.out.println("Process " + runningP.getId() + " finished at time " + endTime + "\n"); running = false;}
